...
- **专家可用工具名单支持留空放行全部**：`expert_tool_names` 留空时不再回退纯文本咨询，而是放行全部全局工具（含 MCP 工具）；委派类（consult/convene）与保留类（send_message_to_user / future_task）安全排除项仍恒生效，动手工具包显式关闭时其名单内的制作类工具一并排除，已停用工具不下发

### 新增

- **专家动手工具包（带开关）**：新增 `expert_creation_tools_enabled`（默认开启），关闭即退回纯信息查询模式；开启时在基础名单外叠加 `expert_creation_tool_names` 制作类工具，让专家可直接参与制作环节而不只是口头建议
- **行动指引升级**：专家系统提示词从"可查证信息"升级为"动手参与制作"——查证事实 → 运行验证 → 动手制作 → 汇报操作与结果，并要求改动克制、可回滚、拒绝无关破坏性操作
- 默认动手包：执行 shell / Python、会话式 shell 管理、文件读/写/改、grep 检索、Tavily 联网搜索与网页提取、知识库检索
- `expert_max_tool_steps` 默认 5 → 8，未手动改过的旧配置自动升级

### 安全

- 保留类工具强制排除：`send_message_to_user`（防专家以 Bot 身份越权发消息）、`future_task`（防咨询结束后遗留定时任务）；无论用户如何配置均不下发并记录警告
- 委派类工具防递归排除机制保持不变；未注册或已停用的工具自动跳过

## [v1.4.1] - 2026-08-25

### 新增

- **专家工具使用白名单（带总开关）**：`expert_tools_whitelist_enabled` 关闭（默认）时白名单完全不生效，所有会话均可使用专家工具；开启后仅 `whitelist_users` / `whitelist_groups` 名单内的私聊用户、群聊或个人命中会话可触发 `consult_expert` / `convene_expert_panel`，管理员自动豁免；未授权会话主模型收到 `[EXPERT_ERROR]` 未授权提示后由主模型自行作答并转告用户
- ID 列表配置兼容列表 / 逗号 / 空格 / 换行分隔

## [v1.4.0] - 2026-08-25

### 新增

- **全局默认模型回退**：新增 `default_provider_id` / `default_model`，配置多位专家无需逐个选模型；回退链为「专家自带 → 全局默认 → 会话当前」；`default_model` 仅在与 `default_provider_id` 配套时生效
- **智能调度（可选）**：开启后每条消息先由调度模型判断是否需要召集专家，需要时自动完成咨询并将意见注入本次请求供主模型融合回答；判断 Provider 留空则使用会话当前模型；任何判断/咨询失败均静默跳过，不影响正常对话
...
- **主动协作触发策略（综合 coder/reviewer 会诊建议）**：为 `consult_expert` 与 `convene_expert_panel` 工具说明新增 PROACTIVE USE 章节——三级触发（明确专业领域直接调用 / 大型任务先向用户确认 / 边缘场景仅推荐 `/experts` 或 `/panel` 指令）；主动信号（自身无把握、多步任务约 3 步以上、发现可疑问题）；配额卫生（每轮对话预留约 5 次保底、不向同一专家重复提问同一问题）；降级预案（调用失败或配额耗尽时自行作答并说明）
